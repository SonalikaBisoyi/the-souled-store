...
'11-12Y','12-14Y','UK3','UK4','UK5','UK6','UK7','UK8'];
  useEffect(() => {
    ...
    const filteredProductsByGender = gender === 'ALL'
      ? filteredProductsBySelectedProducts // Display all products when 'ALL' is selected for gender
      : filteredProductsBySelectedProducts.filter((product) => product.gender === gender);

    // Function to filter products with "solid" in their names
    const filteredProductsBySolid = filteredProductsByGender.filter((product) =>
      product.name.toLowerCase().includes('solid')
    );

    // Function to handle sorting
    let sortedProducts = [...filteredProductsBySolid];
  ...
  }, [selectedProducts, selectedThemes, selectedSizes, selectedPriceRange, sortingOption, gender]);
  ...
  return (
    ...
    <Box className="page-container" marginTop="150px">
      ...
      <Box className="image-container">
        <Image src="https://prod-img.thesouledstore.com/public/theSoul/uploads/themes/4282320230630181041.jpg?format=webp&w=1300&dpr=1.5" alt="Image" />
      </Box>
      <Box flex="1">
  ...
  <AccordionItem>
    {({ isExpanded }) => (
      <>
        ...
        <Link key={product.id} to={`/basics/${product.id}`}>
          <ProductCard
            product={product}
